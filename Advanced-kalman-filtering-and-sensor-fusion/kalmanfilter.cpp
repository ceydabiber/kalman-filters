...
const double GYRO_BIAS_STD = 0.01; 
const double ACCEL_NOISE_STD = 0.1; 
// ----------------------------------------------------------------------- //
...
void KalmanFilter::predictionStep(GyroMeasurement gyro, double dt)
{
    if (isInitialised())
    {
        ...
        // Augment the state (add noise states for gyro bias and acceleration noise)
        int n_x = state.size();
        int n_w = 2; 
        int n_aug = n_x + n_w; 
        ...
        VectorXd x_aug = VectorXd::Zero(n_aug);
        x_aug.head(n_x) = state;
        x_aug(n_x) = 0;  // Initial gyro bias
        x_aug(n_x + 1) = 0; // Initial acceleration noise

        // Augment the covariance matrix
        MatrixXd P_aug = MatrixXd::Zero(n_aug, n_aug);
        P_aug.topLeftCorner(n_x, n_x) = cov;
        P_aug(n_x, n_x) = GYRO_BIAS_STD * GYRO_BIAS_STD;
        P_aug(n_x + 1, n_x + 1) = ACCEL_NOISE_STD * ACCEL_NOISE_STD;

        std::vector<VectorXd> sigma_points = generateSigmaPoints(x_aug, P_aug);
        std::vector<double> sigma_weights = generateSigmaWeights(n_aug);

        // Predict sigma points using the vehicle motion model
        ...
        state = normaliseState(state); // Normalize heading (yaw)
        ...
        // Set updated state and covariance
        setState(state);
        setCovariance(cov);
    }
}
